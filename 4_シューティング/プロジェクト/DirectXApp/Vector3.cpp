...
{
	return Vector3(1.0f, 0.0f, 0.0f);//ワールドX軸
}

Vector3 Vector3::WorldY()
{
	return Vector3(0.0f, 1.0f, 0.0f);//ワールドY軸
}

Vector3 Vector3::WorldZ()
{
	return Vector3(0.0f, 0.0f, 1.0f);//ワールドZ軸
}
...
float Vector3::Dot(Vector3 vector1, Vector3 vector2) {
	return vector1.x * vector2.x + vector1.y * vector2.y + vector1.z * vector2.z;//内積
}
...
float Vector3::Distance(Vector3 vectorA, Vector3 vectorB) {
	return std::sqrt((vectorB.x - vectorA.x) * (vectorB.x - vectorA.x) + (vectorB.y - vectorA.y) * (vectorB.y - vectorA.y) + (vectorB.z - vectorA.z) * (vectorB.z - vectorA.z));//距離
}

float Vector3::DistanceSquare(Vector3 vector) const
{
	return(vector.x - x) * (vector.x - x) + (vector.y - y) * (vector.y - y) + (vector.z - z) * (vector.z - z);//距離の二乗
}

float Vector3::DistanceSquare(Vector3 vector1, Vector3 vector2)
{
	return (vector2.x - vector1.x) * (vector2.x - vector1.x) + (vector2.y - vector1.y) * (vector2.y - vector1.y) + (vector2.z - vector1.z) * (vector2.z - vector1.z);//距離の二乗
}
